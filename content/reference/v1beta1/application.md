...
After updating KOTS to an appropriate version, end users can return to the admin console and click the "Download" button next to the pending version in order to download it. You can also use the [`kots upstream download` command](/kots-cli/upstream/download) to do this. Once the version is downloaded, end users can proceed normally.
